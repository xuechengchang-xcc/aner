...
public abstract class BaseTest {
    ...
    protected  void login(String configFile,String username,String password){
        ...
        Factory<SecurityManager> factor =new IniSecurityManagerFactory(configFile);

        //2.得到SecurityManager实例，并绑定给SecurityUtils
        SecurityManager securityManager =factor.getInstance();
        SecurityUtils.setSecurityManager(securityManager);

        //得到subject以及创建用户名/身份密码验证token(用户身份凭证)
        Subject subject = SecurityUtils.getSubject();
        UsernamePasswordToken token = new UsernamePasswordToken(username,password);

        subject.login(token);
    }
        public Subject subject(){ return SecurityUtils.getSubject(); }
}
